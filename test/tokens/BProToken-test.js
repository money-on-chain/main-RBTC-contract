...
const BProToken = artifacts.require('./contracts/BProToken.sol');

contract('BProToken', function([owner, anotherAccount, ...otherAccounts]) {
  const minter = owner;
  let token;
  const initialBalance = 1000;
  const otherAccountsToTest = otherAccounts.slice(0, 10);
  beforeEach(async function() {
    token = await BProToken.new({ from: owner });
