...
const executeOperations = (user, operations) => {
  const promises = operations.map(async op =>
    mocHelper.mintBProAmount(user, op.reserve)
  );

  return Promise.all(promises);
};

const operationsTotal = operations =>
  operations.reduce((last, op) => op.reserve + last, 0);
    ...
    scenarios.forEach(s => {
      describe(`WHEN proportion is set to ${s.proportion}`, function() {
        before(async function() {
          ...
          await mocHelper.mockMocInrateChanger.setCommissionRate(
            toContractBN(0.002, "RAT")
          );
          // set commissions address
          await mocHelper.mockMocInrateChanger.setCommissionsAddress(
            commissionSplitter.address
          );
          // update params
          await mocHelper.governor.executeChange(
            mocHelper.mockMocInrateChanger.address
          );
        ...
        describe(`WHEN a user uses ${operationsTotal(
          ...
          it(`THEN CommissionSplitter Reserve Token balance should be ${s.commissionAmount +
            ...
            const splitterBalance = await mocHelper.getReserveBalance(
              commissionSplitter.address
            );
            ...
            mocHelper.assertBigReserve(
              ...
              "Commission splitter's reserveToken balance is incorrect"
            );
          });
          ...
          describe("AND WHEN the Splitter contract split function is called", function() {
            ...
            let splitTx;
            before(async function() {
              comAccountInitialBalance = await mocHelper.getReserveBalance(
                commissionsAccount
              );
              mocInitialBalance = await mocHelper.getReserveBalance(
                mocHelper.moc.address
              );
              splitTx = await commissionSplitter.split();
            });
            it("THEN Split event is emitted", async function() {
              const [splitEvent] = await mocHelper.findEvents(
                splitTx,
                "SplitExecuted"
              );
            ...
            it(`THEN the commission address receives ${s.commissionAmount} reserveTokens`, async function() {
              const comAcountBalance = await mocHelper.getReserveBalance(
                commissionsAccount
              );
            ...
            it(`AND moc receives ${s.mocAmount} reserveTokens`, async function() {
              const mocAcountBalance = await mocHelper.getReserveBalance(
                mocHelper.moc.address
              );
              ...
              mocHelper.assertBigReserve(
                mocDiff,
                s.mocAmount,
                "MoC account balance is incorrect"
              );
            });
          });
        });
