...
const MoCSettlementMock_v019 = artifacts.require(
  "./contracts/mocks/MoCSettlementMock_v019.sol"
);
...
const MoCSettlement_v019 = artifacts.require(
  "./contracts/MoCSettlement_v019.sol"
);
...
const createContracts = params => async ({ owner, useMock }) => {
  ...
  const newMoCSettlementImplementation = useMock ? await MoCSettlementMock_v019.new() : await MoCSettlement_v019.new();
  ...
  await governor.executeChange(upgrader.address, { gas: 6e6 });

  const newMoCSettlementProxy = await MoCSettlement_v019.at(
    mocSettlement.address
  );
